...
interface Props {
    variant: 'A' | 'B'; // A: Numbers only (1-2-3), B: Mixed (1-A-2-B)
    nodeCount?: number;
...
export default function TrailMakingTask({ variant, nodeCount = 8, onComplete }: Props) {
    ...
    // 1. Initialize and position nodes (Collision avoid mapping)
    useEffect(() => {
        const labels = generateLabels(variant, nodeCount);
        const newNodes: Node[] = [];
        const nodeSize = 12; // Radius (%)
    ...
    const handleNodeClick = (index: number) => {
        ...
        if (index === nextTargetIndex) {
            ...
            setNodes(currentNodes);
            setTimeout(() => {
                currentNodes[index].status = 'pending';
                setNodes([...currentNodes]);
            }, 500);
        }
    };
    ...
    return (
        <div className="flex flex-col items-center w-full h-full max-w-md mx-auto">
            <h3 className="text-xl font-bold mb-4 text-gray-700 text-center">
                {variant === 'A' ? 'Connect from 1 in order' : 'Alternate: 1 -> A -> 2 -> B...'}
            </h3>
            ...
            <div ref={containerRef} className="relative w-full h-96 bg-gray-50 border-2 border-gray-200 rounded-xl shadow-inner overflow-hidden">
                ...
                {nodes.map((node, i) => (
                    <button
                        ...
                        className={`absolute w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg shadow-md transition-all duration-200 transform focus:outline-none
...
function generateLabels(variant: 'A' | 'B', count: number) {
    ...
    for (let i = 0; i < count; i++) {
        if (variant === 'A') {
            ...
            else result.push(alphabet[Math.floor(i / 2)]);
        }
    }
    return result;
}
