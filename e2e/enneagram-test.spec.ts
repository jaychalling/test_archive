...
test.describe('Enneagram Test - Full Workflow', () => {
  // Increase timeout for many questions
  test.setTimeout(120000);

  test('should complete the entire Enneagram test and show results', async ({ page }) => {
    // 1. Navigate to the test page
    await page.goto('/test/enneagram-test/');

    // 2. Verify landing page elements
    await expect(page.locator('h1')).toContainText('Enneagram');
    ...
    const totalQuestions = totalMatch ? parseInt(totalMatch[1]) : 36;
    ...
    await expect(page).toHaveURL(/\/test\/enneagram-test\/result\/?/);

    // 9. Verify result page shows Enneagram type (1-9)
    await expect(page.locator('text=/Type\\s*[1-9]/i').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should persist answers in localStorage', async ({ page }) => {
    ...
    const answers = await page.evaluate(() => localStorage.getItem('enneagramAnswers'));
