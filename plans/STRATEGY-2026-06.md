...
- **링커블 자산** (✅ 6/12 배포, b9b1cea): `/stats/` — 인구 norm 통계 페이지 (Dataset 스키마, 인용 복사 버튼, 섹션별 테스트 CTA).
- **임베드 위젯** (✅ 6/12 배포, b9b1cea): `/embed-a-test/` — 테스트 10종 iframe 스니펫 + 크레딧 백링크. 테스트 페이지 frame 차단 헤더 없음 확인.
- **소셜 시딩 캐던스** (⬜ 유일한 미실행): Pinterest(퀴즈 핀은 에버그린 복리, 계정 필요 — CEO), 관련 서브레딧(수동 판단), Threads(theads_bot 인프라 재활용 검토). 주 2~3회.
