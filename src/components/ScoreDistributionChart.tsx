...
// Simulate realistic score distribution using normal distribution
const getPercentile = (score: number, maxScore: number): number => {
  // Assume mean is around 60% and standard deviation is 15%
...
const getAverageScore = (maxScore: number): number => {
  // Average is typically around 60% of max
...
const ScoreDistributionChart = ({
  ...
  const averageScore = getAverageScore(maxScore);
  const averagePercentage = (averageScore / maxScore) * 100;
  const percentile = getPercentile(userScore, maxScore);
