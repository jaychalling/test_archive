...
const LoveCompatibilityResult = () => {
  ...
  useEffect(() => {
    const savedAnswers = localStorage.getItem('loveCompatibilityAnswers');
    if (savedAnswers) {
      setAnswers(JSON.parse(savedAnswers));
    } else {
      navigate('/test/love-compatibility-test/');
    }
  }, [navigate]);

  const handleReset = () => {
    localStorage.removeItem('loveCompatibilityAnswers');
    navigate('/test/love-compatibility-test/');
  };
...
${levelInfo.shortDescription}`;
  ...
  const breadcrumbSchema = createBreadcrumbSchema([
    ...
    { name: 'Results', path: '/test/love-compatibility-test/result/' },
  ]);
  ...
  return (
    <div className="min-h-screen gradient-hero">
      <SEOHead
        ...
        path="/test/love-compatibility-test/result/"
...
export default LoveCompatibilityResult;
