...
test.describe('Political Compass Test - Full Workflow', () => {
  test.setTimeout(120000);

  test('should complete the entire Political Compass test and show results', async ({ page }) => {
    // 1. Navigate to the test page
    await page.goto('/test/political-compass-test/');

    // 2. Verify landing page elements
    await expect(page.locator('h1')).toContainText('Political Compass Test');
    await expect(page.getByRole('button', { name: 'Start Test' })).toBeVisible();

    // 3. Start the test
    await page.getByRole('button', { name: 'Start Test' }).click();

    // 4. Verify test started - should show 1/22
    ...
    const totalQuestions = totalMatch ? parseInt(totalMatch[1]) : 22;
    ...
    await expect(page).toHaveURL(/\/test\/political-compass-test\/result\/?/);

    // 9. Verify result page shows quadrant type
    await expect(page.locator('text=/Left Authoritarian|Right Authoritarian|Left Libertarian|Right Libertarian/i').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should persist answers in localStorage', async ({ page }) => {
    ...
    const answers = await page.evaluate(() => localStorage.getItem('politicalCompassAnswers'));
