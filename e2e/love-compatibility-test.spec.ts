...
test.describe('Love Compatibility Test - Full Workflow', () => {
  test.setTimeout(120000);

  test('should complete the entire Love Compatibility test and show results', async ({ page }) => {
    // 1. Navigate to the test page
    await page.goto('/test/love-compatibility-test/');

    // 2. Verify landing page elements
    await expect(page.locator('h1')).toContainText('Love Compatibility');
    await expect(page.getByRole('button', { name: 'Start Test' })).toBeVisible();

    // 3. Start the test
    await page.getByRole('button', { name: 'Start Test' }).click();

    // 4. Verify test started - should show progress
    ...
    await expect(page).toHaveURL(/\/test\/love-compatibility-test\/result\/?/);

    // 9. Verify result page shows compatibility categories
    await expect(page.locator('text=/Communication|Values|Emotional|Lifestyle|Conflict/i').first()).toBeVisible({ timeout: 10000 });
  ...
  test('should persist answers in localStorage', async ({ page }) => {
    ...
    const answers = await page.evaluate(() => localStorage.getItem('loveCompatibilityAnswers'));
